...
class BinaryTree {
    ...
    void processTreeToDeletePipes(Node * tn) {
        if (tn != NULL) {
            processTreeToDeletePipes(tn->left);

            ChildInfo * d = dynamic_cast<ChildInfo*> (tn->data);

            cout << "Deleting " << d->getNorthPipe().c_str() << endl;
            cout << "Deleting " << d->getSouthPipe().c_str() << endl;

            close(d->GetNorthfd());
            close(d->GetSouthfd());

            unlink(d->getNorthPipe().c_str());
            unlink(d->getSouthPipe().c_str());

            processTreeToDeletePipes(tn->right);
        }
    }
    ...
    void processTreeToSendCountries(BinaryTree<ChildInfo> * children, Node * tn) {
        if (tn != NULL) {
            ...
            cout << "Sending " << d->GetName() << " to " << info->GetKey() << " ... " << " southfd: " << info->GetSouthfd() << endl;

            writer(ChildInfo::buffer, d->GetName().c_str(), ChildInfo::buffersize, d->GetName().length() + 1, info->GetSouthfd());
...
public:
    ...
    void processTreeToDeletePipes() {
        processTreeToDeletePipes(this->root);
    }
    ...
    void processTreeToSendCountries(BinaryTree<ChildInfo> * children, BinaryTree<Disease> & diseases, int specific_child) {
        ...
        cout << "Closing: south fd: " << info->GetSouthfd() << endl;
        ...
        char * msg = reader_ack(ChildInfo::buffer, ChildInfo::buffersize, info->GetNorthfd());

        delete [] msg;

        int southfd;

        if ((southfd = open(info->getSouthPipe().c_str(), O_WRONLY)) < 0) {
        ...
        cout << "waiting for viruses from child " << i << " fd: " << info->GetNorthfd() << endl;

        while ((virus = reader(ChildInfo::buffer, ChildInfo::buffersize, info->GetNorthfd())) != NULL) {
        ...
        writer_ack(ChildInfo::buffer, ChildInfo::buffersize, info->GetSouthfd());

        int north;

        if ((north = open(info->getNorthPipe().c_str(), O_RDONLY) < 0)) {
            perror("server: can't open write fifo:");
        }
        cout << "*** Synch complete - PARENT ***\n";
    }

    void processTreeToSendCountries(BinaryTree<ChildInfo> * children, BinaryTree<Disease> & diseases) {
        // send
