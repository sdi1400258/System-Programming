...
void Session::loadFromPipe(string sourceDirectory, int northfd, int southfd) {
    char * country;
    int counter = 0;
    
    cout << "loading from " << sourceDirectory << endl;

    while ((country = reader(ChildInfo::buffer, ChildInfo::buffersize, southfd)) != NULL) {
    ...
    writer_ack(ChildInfo::buffer, ChildInfo::buffersize, northfd);
    ...
    diseases.processTreeToSendDiseases(northfd);
}

void Session::loadFromBST(string sourceDirectory, int northfd, int southfd) {
    cout << "loading from " << sourceDirectory << endl;
